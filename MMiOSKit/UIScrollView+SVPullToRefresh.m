...
- (void)setScrollViewContentInsetForLoading {
    ...
    switch (self.position) {
        case SVPullToRefreshPositionTop:
            ...
            break;
        case SVPullToRefreshPositionBottom:
            currentInsets.bottom = MAX(offset, self.originalBottomInset + self.bounds.size.height);
            break;
    }
    [self setScrollViewContentInset:currentInsets];
}
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 28, 48)];
...
- (void)drawRect:(CGRect)rect {
    UIImage *image = [UIImage imageNamed:@"drag_refresh"];
